...
def test_store_is_namespace_scoped(tmp_path):
    ...
    a, _ = store.create_record(
        namespace="a", environment="production", kind="checkpoint",
        content="same", metadata={}, backends=["hindsight"],
    )
    b, _ = store.create_record(
        namespace="b", environment="production", kind="checkpoint",
        content="same", metadata={}, backends=["hindsight"],
    )
    assert a != b
    store.close()
...
def test_concurrent_idempotency(tmp_path):
    ...
    def create():
        return store.create_record(
            namespace="a", environment="staging", kind="checkpoint",
            content="same", metadata={"x": 1}, backends=["hindsight", "mnemosyne"],
        )[0]
